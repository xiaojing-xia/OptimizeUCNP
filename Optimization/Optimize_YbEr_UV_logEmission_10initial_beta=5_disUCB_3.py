...
LOG_DEST = "../saved_data/fws_YbEr_UV_10initial_logEmission_beta=5_disUCB_3.csv"
FLAG_DEST = "../saved_data/flag_YbEr_UV_10initial_logEmission_beta=5_disUCB_3.csv"
def check_last_run():
...
def run_study():
    ...
    discrete_choices = [yb1,er1,yb2,er2,rdi]
    constraints = [[torch.Tensor([0,1]).long(),torch.Tensor([-1,-1]), -1],[torch.Tensor([2,3]).long(),torch.Tensor([-1,-1]), -1]]
...
def main():
    ...
    parser.add_argument("-c", "--configs", dest="configs",
                       default="common/defaults_YbEr_UV_logEmission_beta=5_3.cfg",
